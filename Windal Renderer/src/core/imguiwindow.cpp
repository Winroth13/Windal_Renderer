...
bool ImguiWindow::Create(Window* window, Renderer* renderer)
{
	...
	if (ImGui_ImplWin32_Init(window->GetWindowsWindow()) == false)
		return false;
	if (ImGui_ImplDX11_Init(renderer->GetDevice(), renderer->GetContext()) == false)
		return false;
...
bool ImguiWindow::ProcessMessages(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
